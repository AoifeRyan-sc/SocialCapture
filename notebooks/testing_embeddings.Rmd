...
# Updates 10 October 2025
 To do:
 - [X] Redirect links
 - [ ] facebook images cropped out - seems to be fixed now even though I haven't changed anything
 - [ ] facebook vids not opening 
 
 
Redirect Links:
Let's look at getting redirect permalinks on links that don't work
```{r}
og_links <- c("https://www.facebook.com/1522553291397399_881348484033562",
                "https://www.facebook.com/1522553291397399_883954233772987",
                "https://www.facebook.com/1522553291397399_887699233398487")

redirected_info <- lapply(og_links, base::curlGetHeaders)
length(redirected_info)
class(redirected_info[[1]])

url_pattern <- "https?://.*"

redirected_links <- sapply(redirected_info, function(x) {
  location_matches <- grep(pattern = "\\blocation\\b", x, value = TRUE, ignore.case = TRUE)
  regmatches(location_matches, regexpr(url_pattern, location_matches))
})

embedded_links <- lapply(redirected_links, embed_switch)
```

```{r}
permalink <- "https://www.facebook.com/1522553291397399_881348484033562"
base::curlGetHeaders(permalink)
```
...
server <- function(input, output) {
  output$renders <- renderUI({
    do.call(bslib::layout_column_wrap, c(width = 1/3, c(embedded_links)))
  })
}
...
But what about if you input a working link?
```{r}
working_links <- c("https://www.facebook.com/casacookhotels/posts/pfbid02YAHjz8wRXJi2VhW4gzgQfhyt5tu5Z3EXFyzrgfKVY8KwAqs6NRrc6KEd1UPy1pJrl",
                   "https://www.facebook.com/casacookhotels/posts/pfbid02rPSfXxmp8x5vNtSkixHSE2HVUKTTKk5Br7ygjwoSiPNEVd7898QfD9ahCtKT3Vtkl",
                   "https://www.facebook.com/casacookhotels/posts/pfbid0tNfHrzvD16boe8JBVDh2uTP3ZdZeDEEWUEFuCUXZ9J7jyKABG9FNHvrE8hjTMJUdl")

redirected_info <- lapply(working_links, base::curlGetHeaders)
length(redirected_links)
class(redirected_links[[1]])
stringr::str_detect(redirected_info[[1]], "(?i)\\blocation")
```

so if no hit on "location" - use og permalink

This runs into bother when deployed as the `curlGetHeaders` function returns links with "unsupportedbrowser" for meta sites. I have looked at using `httr`s `GET` and `HEAD` but neither return the redirect link. 

Trying the actual curl package, creds to ChatGPT...
```{r}
library(curl)

get_custom_headers <- function(url, custom_headers = list()) {
  h <- new_handle()
  
  handle_setopt(h,
                followlocation = TRUE, 
                useragent = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/58.0.3029.110 Safari/537.3")  # Mimic a real browser
  
  for (header in names(custom_headers)) {
    handle_setheaders(h, sprintf("%s: %s", header, custom_headers[[header]]))
  }
...
# Test the function
url <- "https://www.facebook.com/1522553291397399_881348484033562"
custom_headers <- list(
  "Accept-Language" = "en-US,en;q=0.9",
  "Custom-Header" = "CustomValue"  # Example of a custom header
)

result <- get_custom_headers(url, custom_headers)

cat("Redirect URL:", result$last_url, "\n")
cat("Response Headers:\n", result$headers, "\n")

```

I also tried the Meta Graph API from the Graph API Explorer but you need to get meta review and clearance to use the API which seems a bit of a faff. 
